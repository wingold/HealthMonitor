...
['validation-failed' , 'Validation failed.'],
	...
	['rate-validate' , 'Please enter correct rate'],
	...
	['two-point-number','Input value must be two point number value.'],
	...
	['validate-alphanumch' , '请输入英文字母或是数字或汉字等,特殊字符是不允许的.'],
...
Validation.addAllThese([
		...
		['validate-alphanumch', function(v) {
			return !/\W/.test(v)
		}],
		...
		['validate-alphanumch',/^[a-zA-Z0-9\u4e00-\u9fa5-_().*]+$/],
		...
		Validation.addAllThese([
				 ...
				 * Usage : less-than-equal-$otherInputId
				 */
				['less-than-equal-settlementprice', function(v,elm,args,metadata) {
					var compareValue = $('#' + args[0]).val();
					if(Validation.get('validate-number').test(v) && Validation.get('validate-number').test(compareValue))
						return parseFloat(v) <= parseFloat(compareValue);
					return v < compareValue || v == compareValue;
				 ...
				 */
				['two-point-number', function(v,elm,args) {
					return Validation.get('validate-number').test(v) && /^\d{1,8}(?:\.\d{1,2})?$/.test(v);
				}],
				/**
				 * Usage : great-than-equal-$otherInputId
				 */
				 ...
				 * Example: -2.1 to 3 = float-range--2.1-3
				 */
				['rate-validate',function(v,elm,args,metadata) {
